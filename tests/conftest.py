...
from deeptable import Deeptable, AsyncDeeptable, DefaultAioHttpClient
from deeptable._utils import is_dict
...
logging.getLogger("deeptable").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Deeptable]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Deeptable(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncDeeptable]:
